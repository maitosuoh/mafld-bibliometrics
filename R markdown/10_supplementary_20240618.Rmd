...
```{r}
# load 09_flow_chart
load(file="rdata/09_flow_chart.RData")
```

```{r}
# plot ciation counts in scopus (Fig S2)
s2_a <- df_bx %>%
  filter(!is.na(TC_s)) %>%
  slice_max(TC_s, n = 15) %>%
...
```{r}
# plot citation counts in wos (Fig S2)
s2_b <- df_bx %>%
  filter(!is.na(TC_w)) %>%
  slice_max(TC_w, n = 15) %>%
  mutate(PN2 = case_when(PM == "32278004" ~ paste(PN2, "[a]"),
...
```{r}
# Fig S2: combine plots and save as pdf
s2 <- s2_a / s2_b
s2 <- s2 +
    labs(caption = "<span style = 'font-family:sans'><b>Fig. S2 a, b</b></span> Highly cited MAFLD papers in Scopus (<b>a</b>) and Web of Science (<b>b</b>).")+
    theme(plot.caption = element_markdown(family = "serif", size = 10, hjust = 0),
          plot.caption.position =  "plot")
ggsave("figure/FigS2.pdf", s2, width = 210, height = 297, unit = "mm")
```
...
```{r}
# plot common keywords in group A (Fig S3)
s3_a <- df_de_cr %>% filter(group == "A") %>% count(DE) %>% slice_max(n, n = 15) %>%
...
```{r}
# plot common keywords in group B (Fig S3)
s3_b <- df_de_cr %>% filter(group == "B") %>% count(DE) %>% slice_max(n, n = 15) %>%
  mutate(DE = str_to_sentence(DE)) %>%
  mutate(DE = case_when(DE == "Mafld" ~ "MAFLD",
...
```{r}
# plot common keywords in group C (Fig S3)
s3_c <- df_de_cr %>% filter(group == "C") %>% count(DE) %>% slice_max(n, n = 15) %>%
  mutate(DE = str_to_sentence(DE)) %>%
  mutate(DE = case_when(DE == "Mafld" ~ "MAFLD",
                        DE == "Nhanes" ~ "NHANES",
                        .default = as.character(DE))) %>%
...
```{r}
# plot common keywords in group D (Fig S3)
s3_d <- df_de_cr %>% filter(group == "D") %>% count(DE) %>% slice_max(n, n = 15) %>%
  mutate(DE = str_to_sentence(DE)) %>%
  mutate(DE = case_when(DE == "Mafld" ~ "MAFLD",
                        .default = as.character(DE))) %>%
  mutate(de_width = str_width(DE)) %>%
  mutate(de_first = str_sub(DE, 1, 1)) %>%
  arrange(n, desc(de_width), desc(de_first)) %>%
  mutate(order = row_number()) %>%
  ggplot(aes(n, fct_reorder(DE, order)))+
...
```{r}
# plot common keywords in group E (Fig S3)
s3_e <- df_de_cr %>% filter(group == "E") %>% count(DE) %>% slice_max(n, n = 15) %>%
  mutate(DE = str_to_sentence(DE)) %>%
  mutate(DE = case_when(DE == "Mafld" ~ "MAFLD",
...
```{r}
s3 <- s3_a + s3_b + s3_c + s3_d + s3_e +
  plot_layout(design = "
  AB
  CD
  E#")+
  labs(caption = "<span style = 'font-family:sans'><b>Fig. S3 a-e</b></span> Frequent keywords in each publication group (A-E) of the co-citation network created from MAFLD<br>literature.")+
  theme(plot.caption = element_markdown(family = "serif", size = 10, hjust = 0,
                                        lineheight = 1.25),
        plot.caption.position =  "plot")
...
```{r}
s4 <- s4+
  labs(caption = "<span style = 'font-family:sans'><b>Fig. S4</b></span> Top countries producing MAFLD literature.")+
  theme(plot.caption = element_markdown(family = "serif", size = 10, hjust = 0),
        plot.caption.position =  "plot")
...
```{r}
save.image("10_supplementary.RData")
```
