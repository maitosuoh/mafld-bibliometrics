...
```{r}
masld_search <- masld %>% str_to_upper() %>% paste0(collapse = "|")
masld_search 
```

```{r}
df_bx %>% filter(str_detect(TI, masld_search ) | str_detect(AB, masld_search )) %>% nrow()
```
...
df_bx %>% filter(str_detect(TI, "GUIDELINE|GUIDANCE") & DT_p != "REVIEW") %>% select(TI, PM, DT_p, DT_w, DT_s)
# 33604249 is editorial; change later
...
df_bx <- df_bx %>% 
  mutate(DT_p = case_when(str_detect(TI, "REVIEW|GUIDELINE|GUIDANCE") & DT_p != "REVIEW" ~ "REVIEW",
                          str_detect(TI, "COMMENT") & DT_p != "EDITORIAL" ~ "EDITORIAL",
                          PM %in% c("37651223", "37602206") ~ "EDITORIAL",
                          str_detect(TI, "REPLY") & DT_p != "EDITORIAL" ~ "EDITORIAL",
                          .default = as.character(DT_p))) %>%
  mutate(DT_p = case_when(PM == "33604249" ~ "EDITORIAL",
                          .default = as.character(DT_p)))
```

```{r}
df_bx %>% filter(PM == "33604249")  %>% select(TI, PM, DT_p, DT_w, DT_s)
```
...
df_bx %>% filter(str_detect(TI, "GUIDELINE|GUIDANCE") & DT_w != "REVIEW") %>% select(TI, PM, DT_p, DT_w, DT_s)
...
df_bx <- df_bx %>% 
  mutate(DT_w = case_when(str_detect(TI, "REVIEW|GUIDELINE|GUIDANCE") & DT_w != "REVIEW" ~ "REVIEW",
...
df_bx %>% filter(str_detect(TI, "GUIDELINE|GUIDANCE") & DT_s != "REVIEW") %>% select(TI, PM, DT_p, DT_w, DT_s)
...
df_bx <- df_bx %>% 
  mutate(DT_s = case_when(str_detect(TI, "REVIEW|GUIDELINE|GUIDANCE") & DT_s != "REVIEW" ~ "REVIEW",
                          str_detect(TI, "COMMENT") & DT_s != "EDITORIAL" ~ "EDITORIAL",
                          str_detect(TI, "EDITORIAL") & DT_s != "EDITORIAL" ~ "EDITORIAL",
                          .default = as.character(DT_s))) %>%
  mutate(DT_s = case_when(PM == "33604249" ~ "EDITORIAL",
                          .default = as.character(DT_s)))
```
...
```{r}
# assign two matches in REVIEW

df_bx <- df_bx %>%
  mutate(DT = case_when( is.na(DT) & (DT_p == "REVIEW" & DT_w == "REVIEW") ~ "REVIEW",
                         is.na(DT) & (DT_p == "REVIEW" & DT_s == "REVIEW") ~ "REVIEW",
                         is.na(DT) & (DT_w == "REVIEW" & DT_s == "REVIEW") ~ "REVIEW",
                         .default = as.character(DT)))
```

```{r}
# assign two matches in EDITORIAL
df_bx <- df_bx %>%
  mutate(DT = case_when( is.na(DT) & (DT_p == "EDITORIAL" & DT_w == "EDITORIAL") ~ "EDITORIAL",
                         is.na(DT) & (DT_p == "EDITORIAL" & DT_s == "EDITORIAL") ~ "EDITORIAL",
                         is.na(DT) & (DT_w == "EDITORIAL" & DT_s == "EDITORIAL") ~ "EDITORIAL",
                         .default = as.character(DT)))
```
...
```{r}
df_bx %>% filter( is.na(DT) & (DT_p == DT_w & DT_p != DT_s) ) %>% select(TI, PM, DT, UI, DT_p, DT_w, DT_s)

# 38577539 article based on agreement with pubmed and wos
#	33534815 article
```

```{r}
# fill ARTICLE
df_bx <- df_bx %>% mutate(DT = case_when( is.na(DT) & (DT_p == DT_w & DT_p != DT_s) ~ "ARTICLE",
                           .default = as.character(DT)))
```


```{r}
df_bx %>% filter( is.na(DT) & (DT_p == DT_s & DT_p != DT_w) ) %>% select(TI, PM, DT, UI, PY_p, DT_p, DT_w, DT_s)
# use DT_w
```
```{r}
df_bx <- df_bx %>% mutate(DT = case_when(is.na(DT) & (DT_p == DT_s & DT_p != DT_w)  ~ DT_w,
                           .default = as.character(DT)))
```

```{r}
df_bx %>% filter( is.na(DT) & (DT_w == DT_s & DT_p != DT_w) ) %>% select(TI, PM, DT, UI, DT_p, DT_w, DT_s)
```

```{r}
df_bx <- df_bx %>% mutate(DT = case_when(is.na(DT) & (DT_w == DT_s & DT_p != DT_w)  ~ DT_p,
                           .default = as.character(DT)))
```
...
```{r}
# two matches in ARTICLE -> assign ARTICLE
df_bx <- df_bx %>%
  mutate(DT = case_when( is.na(DT_s) & DT_p == "ARTICLE" & DT_w == "ARTICLE" ~ "ARTICLE",
                         is.na(DT_w) & DT_p == "ARTICLE" & DT_s == "ARTICLE" ~ "ARTICLE",
                         is.na(DT_p) & DT_w == "ARTICLE" & DT_s == "ARTICLE" ~ "ARTICLE",
                                 .default = as.character(DT)))
```

```{r}
# only one DT -> use this
df_bx <- df_bx %>% 
  mutate(DT = case_when(is.na(DT) & !is.na(DT_p) & is.na(DT_w) & is.na(DT_s) ~ DT_p,
                        is.na(DT) &  is.na(DT_p) & !is.na(DT_w) & is.na(DT_s) ~ DT_w,
                        is.na(DT) &  is.na(DT_p) & is.na(DT_w) & !is.na(DT_s) ~ DT_s,
                        .default = as.character(DT)))
```
...
```
```{r}
df_bx %>% filter(is.na(DT)) %>% select(TI, PM, DI, DT, DT_p, DT_w, DT_s)
```

```{r}
df_bx <- df_bx %>%
  mutate(DT = case_when(is.na(DT) ~ DT_p,
                               .default = as.character(DT))) %>%
  mutate(DT = case_when(PM == "34509272" ~ "EDITORIAL",
                        .default = as.character(DT))) %>%
  mutate(DT = case_when(is.na(DT) ~ DT_w,
                        .default = as.character(DT)))
```
...
p1_b <-
  ...
  mutate(DT = factor(DT, levels = c("<b>Article</b> (816)",
                                    "<b>Review</b> (439)",
                                    "<b>Editorial</b> (214)"))) %>%
...
```{r}
df_bx %>% filter(str_detect(SO, "JOURNAL OF HEPATOLOGY")) %>% filter(is.na(TC_w) | is.na(TC_s)) %>% View()
```

```{r}
# journal citation counts in wos
df_bx %>% 
  filter(!is.na(SO_w)) %>%
  group_by(SO) %>%
  summarise(TC_sum = sum(TC_w)) %>%
  slice_max(TC_sum, n = 19) %>%
  mutate(SO = str_remove(SO, ":.*")) %>%
  mutate(SO = str_remove(SO, "\\(.*\\)")) %>%
  mutate(SO = str_remove(SO, "\\.")) %>%
  mutate(SO = str_to_title(SO)) %>%
  mutate(SO = str_replace_all(SO, " Of ", " of ")) %>%
  mutate(SO = str_replace_all(SO, " And ", " and ")) %>%
  mutate(SO = str_replace_all(SO, " In ", " in ")) %>%
  ggplot(aes(TC_sum, fct_reorder(SO, TC_sum)))+
  geom_col(fill = "#f781bf", width = 0.7)+
  labs(x = "Total citation count in Web of Science", y = "Journal")+
...
```{r}
# Journal citation counts in Scopus
df_bx %>% 
  filter(!is.na(SO_s)) %>%
  group_by(SO) %>%
  summarise(TC_sum = sum(TC_s)) %>%
  slice_max(TC_sum, n = 19) %>%
  mutate(SO = str_remove(SO, ":.*")) %>%
  mutate(SO = str_remove(SO, "\\(.*\\)")) %>%
  mutate(SO = str_remove(SO, "\\.")) %>%
  ...
  ggplot(aes(TC_sum, fct_reorder(SO, TC_sum)))+
  geom_col(fill = "#f781bf", width = 0.7)+
  labs(x = "Total citation count in Scopus", y = "Journal")+
  scale_x_continuous(breaks = seq(0, 3000, 500), 
                   limits = c(0,3000), 
                   expand = c(0,0))+
  theme_classic()+
