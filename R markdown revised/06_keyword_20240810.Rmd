...
df_de <- df_bx %>% select(TI, DE, SO, J9, PY, DB, PN2, PN3, PM, DI, UI)
```
...
p3_a <- df_de_long %>%
  ...
  mutate(DE = case_when(DE == "Mafld" ~ "MAFLD",
                        ...
                        DE == "Diabetes mellitus" ~ "Diabetes",
                        DE == "Type 2 diabetes mellitus" ~ "Type 2 diabetes",
                        #DE == "Cardiovascular disease" ~ "CVD",
                        DE == "Covid-19" ~ "COVID-19",
                        .default = as.character(DE))) %>%
...
```{r}
# load p2 from NAFLD analysis
p3_b <- readRDS("rds_nafld/p3_b")
p3_b <- p3_b+labs(tag = "b")
```
