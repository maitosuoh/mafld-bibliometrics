...
df_bx <- pubmed_mafld_ui %>% 
  select(AU, AF, TI, AB, SO, J9, PY, DT, C1, DE, AU_UN, DB, PM, DI, UI) %>%
  full_join(wos_mafld_ui %>% select(AU, AF, TI, AB, SO, J9, PY, DT, C1, DE, AU_UN, TC, CR, DB, DI, PM, UI),
            by = "UI",
            suffix = c("_p", "_w")) %>%
  full_join(scopus_mafld_ui %>% select(AU, AF, TI, AB, SO, J9, PY, DT, C1, DE, AU_UN, TC, CR, DB, DI, PM, UI),
            by = "UI",
            suffix = c("_w","_s")) %>% 
  rename(all_of(c(AU_s = "AU", AF_s = "AF", TI_s = "TI", AB_s = "AB", SO_s = "SO", J9_s = "J9", PY_s = "PY", DT_s = "DT", C1_s = "C1", DB_s = "DB", DE_s = "DE", AU_UN_s = "AU_UN", DI_s = "DI", PM_s = "PM")))
```
